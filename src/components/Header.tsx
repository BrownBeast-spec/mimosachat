...
const Header = () => {
  ...
  return (
    <header className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] md:w-[80%] max-w-5xl transition-all duration-300">
      <div className={`rounded-2xl md:rounded-full px-4 md:px-8 py-3 md:py-4 shadow-elevated transition-all duration-500 ${
        isScrolled 
        ...
        <div className="flex items-center justify-between">
          ...
          <div className={isMobile ? "" : "absolute left-1/2 -translate-x-1/2"}>
            ...
            }`} style={{fontFamily: 'Poppins, sans-serif'}}>Mimosa</span>
          </div>

          {/* Right Register Button */}
          <button className={`rounded-full px-4 md:px-6 py-2 md:py-2.5 flex items-center gap-2 text-sm font-medium transition-all duration-300 shadow-card hover:shadow-soft group ${
            isScrolled 
        ...
        {isMobile && isMenuOpen && (
          <div className={`mt-4 pt-4 transition-colors duration-300 ${
            isScrolled ? 'border-t border-border/30' : 'border-t border-white/20'
          }`}>
